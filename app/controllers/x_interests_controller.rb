class XInterestsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @x_interest.update(x_interest_params)
        format.html { redirect_to x_interest_path, notice: "X interest was successfully updated.", status: :see_other }
      else
  ...
  private
    ...
    def set_x_interest
      @x_interest = current_user.x_interest!
    end

    # Only allow a list of trusted parameters through.
    def x_interest_params
      params.expect(x_interest: [ :description, :user_id ])
    end
end
